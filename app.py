...
"""
EchoAI Streamlit Interface with Aspect-Based Sentiment Analysis
Interactive UI for review sentiment analysis and response generation
"""
import streamlit as st
import pandas as pd
...
def load_model(llm_model='google/flan-t5-base', load_llm=True):
    ...
    # Disable LLM if transformers not available
    if load_llm and not TRANSFORMERS_AVAILABLE:
        st.warning("⚠️ LLM features disabled. Using template responses.")
        load_llm = False
    ...
    with st.spinner('🔄 Loading models... This may take a moment...'):
        try:
            ...
            if load_llm:
                st.success('✅ Models loaded with LLM!')
            else:
                st.success('✅ Models loaded (Template responses)')
            return True
        except Exception as e:
            st.error(f'❌ Error loading models: {str(e)}')
            import traceback
...
def display_aspect_analysis(review_text):
    """Display aspect-based sentiment breakdown"""
    try:
        from AspectSA.aspectsentiment import AspectSentimentAnalyzer
        ...
        st.caption("See how customers feel about specific aspects")
        ...
        for col, aspect in zip(cols, aspects):
            aspect_data = analysis['aspect_sentiments'][aspect]
            
            with col:
                if aspect_data['sentiment'] != 'not_mentioned':
                    # Color based on sentiment
                    if aspect_data['sentiment'] == 'positive':
                        emoji = "✅"
                        color = "#27ae60"
                        bg_color = "#d5f4e6"
                    elif aspect_data['sentiment'] == 'negative':
                        emoji = "❌"
                        color = "#e74c3c"
                        bg_color = "#fadbd8"
                    else:
                    ...
                    st.markdown(f"""
                        <div class="aspect-card" style="background-color: {bg_color};">
                            <h4 style="margin: 0; color: {color};">{emoji} {aspect.capitalize()}</h4>
                            <p style="font-size: 1.2rem; font-weight: bold; color: {color}; margin: 0.5rem 0;">
                                {aspect_data['sentiment'].upper()}
                            </p>
                            <p style="font-size: 0.9rem; color: #7f8c8d; margin: 0;">
                                Score: {aspect_data['score']:.2f} | Confidence: {aspect_data['confidence']:.1%}
                            </p>
                        </div>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown(f"""
                        <div class="aspect-card" style="background-color: #f8f9fa;">
                            <h4 style="margin: 0; color: #bdc3c7;">{aspect.capitalize()}</h4>
                            <p style="font-size: 0.9rem; color: #95a5a6; margin: 0.5rem 0;">
                                Not mentioned
                            </p>
                        </div>
                    """, unsafe_allow_html=True)
        ...
        # Show aggregate info
        st.info(f"💡 **Aspects mentioned:** {analysis['aspects_mentioned']} out of 5 | "
                f"**Overall aspect sentiment:** {analysis['aggregate_sentiment'].upper()} "
                f"(score: {analysis['aggregate_score']:.2f})")
        
    except Exception as e:
        st.warning(f"⚠️ Aspect analysis unavailable: {str(e)}")
...
def display_response(result):
    ...
    if result and 'generated_response' in result:
        ...
        st.markdown("""
            <div class="response-box">
                <h3 style="margin-top: 0; color: #2c3e50;">📝 Generated Response</h3>
                <p style="font-size: 1.1rem; line-height: 1.6; color: #2c3e50; font-weight: 500;">
                    {}
                </p>
            </div>
        """.format(response_text), unsafe_allow_html=True)
        
        # Also show in a text area for easy copying
        with st.expander("📋 Copy Response"):
            st.text_area("Response Text", response_text, height=100, key=f"response_{result.get('timestamp', '')}")
...
def main():
    ...
    if not st.session_state.model_loaded:
        ...
        # Create tabs
        tab1, tab2 = st.tabs(["✏️ Single Review", "📊 Batch Processing"])
            ...
            if st.button("🔍 Analyze Review", type="primary", disabled=not review_text):
                with st.spinner("Processing review..."):
                    ...
                    if result:
                        ...
                        # Display generated response PROMINENTLY
                        display_response(result)
                        
                        # Display aspect analysis
                        display_aspect_analysis(review_text)
                        
                        # Show raw JSON in expander (at bottom)
                        with st.expander("🔧 View Raw Results"):
                            st.json(result)
        ...
        with tab2:
            ...
            uploaded_file = st.file_uploader(
                "Upload CSV file with reviews",
                type=['csv'],
                help="CSV must contain 'reviewText' column. Optional: placeName, placeAddress, provider, reviewRating, authorName, reviewDate"
            )
